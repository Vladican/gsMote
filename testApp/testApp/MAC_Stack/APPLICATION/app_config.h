...
typedef enum
{
	...
	/** Application timer id used indicate device is ready via green LED */
	TIMER_HEARTBEAT = 	(APP_FIRST_TIMER_ID + 0),
    /** Application timer id used to switch off LED */
    TIMER_LED_OFF = 	(APP_FIRST_TIMER_ID + 1),
	/** Application timer id used within ping pong state machine */
	TIMER_PING_PONG =	(APP_FIRST_TIMER_ID + 2),
	/** Application timer id used to sample the module's voltage */
	VOLTAGE_READ	=	(APP_FIRST_TIMER_ID + 3)
} app_timer_t;
// ^
