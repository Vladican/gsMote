 ...
 *
 * $Id: mac_scan.c 20067 2010-01-28 12:35:18Z sschneid $
 *
...
/*
 * SCAN command request payload length
 */
#define SCAN_CMD_PAYLOAD_LEN            (1)
...
bool send_scan_cmd(bool beacon_req)
{
    ...
    /*
     * mac_scan_cmd_buf_ptr holds the small buffer allocated for sending beacon
    ...
    /* Destination PANid */
    transmit_frame->dest_panid = BROADCAST;

    /* Destination address */
    transmit_frame->dest_address = BROADCAST;
    ...
    transmit_frame->payload_length = SCAN_CMD_PAYLOAD_LEN;
    /* end MHR */
    ...
    if (beacon_req)
    {
        transmit_frame->payload[0] = BEACONREQUEST;
    }
    else
    {
        transmit_frame->payload[0] = ORPHANNOTIFICATION;
    }
...
#endif /* (MAC_SCAN_ACTIVE_REQUEST_CONFIRM == 1)) */
#if (MAC_SCAN_ORPHAN_REQUEST_CONFIRM == 1)
    if (MAC_SCAN_ORPHAN == mac_scan_state)
    {
        if (MAC_SUCCESS == status)
        {
            ...
            response_time = mac_pib_macResponseWaitTime;
            response_time = TAL_CONVERT_SYMBOLS_TO_US(response_time);
...
void tal_ed_end_cb(uint8_t energy_level)
{
     ...
     */
    msc = (mlme_scan_conf_t *)BMM_BUFFER_POINTER((buffer_t *) \
            mac_conf_buf_ptr);
