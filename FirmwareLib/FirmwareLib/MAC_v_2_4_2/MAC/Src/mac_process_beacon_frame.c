 ...
 *
 * $Id: mac_process_beacon_frame.c 20147 2010-02-02 14:36:39Z sschneid $
 *
...
void mac_process_beacon_frame(buffer_t *beacon)
{
    ...
    if (MAC_SCAN_IDLE == mac_scan_state)
    {
...
#endif /* MAC_START_REQUEST_CONFIRM */
        {
             ...
             */
            beacon_order =
                GET_BEACON_ORDER(mac_parse_data.payload_data.beacon_data.superframe_spec);
...
#endif
            superframe_order =
                GET_SUPERFRAME_ORDER(mac_parse_data.payload_data.beacon_data.superframe_spec);
#if (DEBUG > 1)
            set_status =
#endif
            ...
            mac_final_cap_slot =
            GET_FINAL_CAP(mac_parse_data.payload_data.beacon_data.superframe_spec);
     ...
     */
    {
        ...
        pand_long.GTSPermit      = mac_parse_data.payload_data.beacon_data.gts_spec >> 7;
    ...
    numaddrshort = NUM_SHORT_PEND_ADDR(
        mac_parse_data.payload_data.beacon_data.pending_addr_spec);

    numaddrlong = NUM_LONG_PEND_ADDR(
        mac_parse_data.payload_data.beacon_data.pending_addr_spec);
#endif
     ...
     * generated
     */
    if ((mac_parse_data.payload_length > 0) || (!mac_pib_macAutoRequest))
    {
        uint8_t index;
        mlme_beacon_notify_ind_t *mbni;
        mbni = (mlme_beacon_notify_ind_t *)BMM_BUFFER_POINTER(((buffer_t *)beacon));
        ...
        mbni->data[0] = mac_parse_data.payload_length + index - 1;
...
/* Handling of presented indirect traffic by the partent for this node. */
    ...
    if (MAC_SCAN_IDLE == mac_scan_state)
    {
        ...
        if (mac_pib_macAutoRequest)
        {
            if (MAC_SYNC_NEVER != mac_sync_state)
            {
                ...
                {
                    if (mac_parse_data.payload_data.beacon_data.
                        pending_addr_list_short[index] == tal_pib_ShortAddress)
                    {
                        /*
                ...
                {
                    if (mac_parse_data.payload_data.beacon_data.
                        pending_addr_list_long[index] == tal_pib_IeeeAddress)
                    {
                        /*
