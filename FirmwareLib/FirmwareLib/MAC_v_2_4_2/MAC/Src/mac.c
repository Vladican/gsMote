 ...
 *
 * $Id: mac.c 18939 2009-10-28 14:37:34Z sschneid $
 *
 ...
 */
uint8_t mac_pib_macSecurityEnabled;
...
/**
 * Holds the Private MAC PIB attribute which can simulate queue
 * overflow condition.
 */
...
/**
 * Holds the MSDU handle of the direct data and is used to give the
 * confirmation of the same. For an indirect data this is a dummy vaiable
 * as for indirect data the msdu handle is present implicitly as a part of
 * the indirect_data_t structure.
 */
uint8_t mac_msdu_handle;
...
/**
 * Flag to indicate if the frame sent to the TAL is an indirect or a direct
 * frame.
 */
bool mac_indirect_flag;

/**
 * Structure used to hold the persistence time, buffer pointer of the indirect
 * data.
 */
indirect_data_pers_timer_t mac_indirect_array[INDIRECT_DATA_QUEUE_CAPACITY];
#endif /* (MAC_INDIRECT_DATA_FFD == 1) */
