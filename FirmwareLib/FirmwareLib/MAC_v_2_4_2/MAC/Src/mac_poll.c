 ...
 *
 * $Id: mac_poll.c 19521 2009-12-11 18:56:53Z sschneid $
 *
 ...
 *
 * @param buf Buffer to send poll confirmation to NHLE.
 * @param status MLME Poll request status.
 */
static void gen_mlme_poll_conf(buffer_t *buf, uint8_t status)
{
    mlme_poll_conf_t *mpc = (mlme_poll_conf_t *)BMM_BUFFER_POINTER(buf);
...
void mlme_poll_request(uint8_t *m)
{
       ...
       )
    {
        ...
        {
#if (DEBUG > 1)
            ASSERT("Cannot build data req cmd" == 0);
#endif
