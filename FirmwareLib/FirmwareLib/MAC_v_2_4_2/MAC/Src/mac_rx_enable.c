 ...
 *
 * $Id: mac_rx_enable.c 20067 2010-01-28 12:35:18Z sschneid $
 *
...
void mlme_rx_enable_request(uint8_t *m)
{
    ...
    else
    {
        ...
        symbols_remaining_in_cap = ((TAL_GET_SUPERFRAME_DURATION_TIME(tal_pib_SuperFrameOrder)) /
                                   aNumSuperframeSlots) *
                                   (mac_final_cap_slot + 1);

        symbols_remaining_in_cap -= symbols_since_beacon;

        if (symbols_remaining_in_cap < rx_off_time_symbols)
        {
            /*
            ...
            if (!(rxe->DeferPermit))
            {
                gen_rx_enable_conf((buffer_t *)m, (uint8_t)MAC_OUT_OF_CAP);
                return;
            }
            else
            {
                 ...
                 */
                rxe->RxOnTime = curr_beacon_int_time_symbols + rxe->RxOnTime;
            }
        }
        ...
        {
            /*
             * Timer to indicate start of RxEnable-time could not be started
             * because timeout was to short to be started even if valid,
             * so go to the corresponding call back function immediately.
             */
             mac_t_rx_on_cb(m);
        }
