 ...
 *
 * $Id: tal_pib.c 19780 2010-01-07 09:19:45Z jwunsch $
 *
...
// LSR
static uint8_t ConvertTransmitPowerToRegisterValue(uint8_t u8TxPowerLevel);
// ^
...
// LSR
        ...
        reg_value = ConvertTransmitPowerToRegisterValue(tal_pib_TransmitPower);
// ^
...
{
	uint8_t u8TxPowerLevelRegisterValue;

	if(u8TxPowerLevel > RF_TX_POWER_LEVEL_MAX)
	{
		u8TxPowerLevel = RF_TX_POWER_LEVEL_MAX;
	}

	u8TxPowerLevelRegisterValue = PGM_READ_BYTE(&tx_pwr_table_NA[RF_TX_POWER_LEVEL_MAX - u8TxPowerLevel]);

	return u8TxPowerLevelRegisterValue;
}
// ^
